...
mod dynamic_rng {
    ...
    #[test]
    fn dynamic_rng_alias() {
        dynamic_alias_rng!(Salt, Vec<u8>); // ← now correct!

        let s1 = Salt::rng(32);
        let s2 = Salt::rng(32);

        assert_eq!(s1.expose_secret().len(), 32);
        assert_eq!(s2.expose_secret().len(), 32);
...
fn raw_rng_types_work() {
    // Fixed-size works perfectly
    let a = FixedRng::<16>::rng();
    let b = FixedRng::<16>::rng();
    assert_ne!(a.expose_secret(), b.expose_secret());

    // Dynamic — must specify the container type
    let c: DynamicRng = DynamicRng::rng(64);
    assert_eq!(c.expose_secret().len(), 64);

    // Or use a type alias (preferred)
    type MyToken = DynamicRng;
    let d = MyToken::rng(128);
